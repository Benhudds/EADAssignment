...
public class HomeController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<h1>Welcome to the geometry drawing application!</h1>");
            if (!loginHelper.ValidateRequest(request)) {
                out.println("<h3>Please log in to continue");
            } else {
                out.println("<h3>Start by drawing a shape, then answer some questions.</h3>");
            }
