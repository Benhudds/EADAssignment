...
public class ShapeServletBase extends ServletBase {
            ...
            switch (acceptHeader) {
                ...
                default:
                    try {
                        ...
                        response.sendRedirect(request.getContextPath() + "/draw/" + params.getShape().getName());
                    }
                    break;
            }
    ...
    public static void processHTMLRequest(HttpServletRequest request, HttpServletResponse response, ShapeParams shapeParams, boolean post,  boolean loggedIn, boolean teacher) throws IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (post) {
                ...
                out.println("<img src='" + CreateImgUrl(request, shapeParams.getShape()) + "'>");
            } else {
                out.println("<img src='" + request.getContextPath() + "/draw/" + shapeParams.getShape().getName() + "'>");
            }
    ...
    public static String CreateImgUrl(HttpServletRequest request, Shape shape) {
        String imageUrl = request.getContextPath() + "/draw/" + shape.getName() + "?";
