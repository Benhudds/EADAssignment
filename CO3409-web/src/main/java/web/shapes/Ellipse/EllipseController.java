...
import imagehelpers.EllipseHelper;
...
@WebServlet(name = "EllipseController", urlPatterns = {"/draw/ellipse"})
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (acceptHeader) {
            case "image/webp,image/apng,image/*,*/*;q=0.8":
                ImageHelper h = null;
                try {
                    ShapeParams shapeParams = ShapeServletBase.validateShape(request, response, Ellipse.class);
                    if (shapeParams.isValid()) {
                        h = new EllipseHelper();
                        ((EllipseHelper) h).Draw((Ellipse) shapeParams.getShape());
                ...
                break;
            default:
                ShapeServletBase.processHTMLRequest(request, response, new ShapeParams().setNewShape(new Ellipse()), false, loginHelper.ValidateRequest(request), loginHelper.ValidateTeacher(request));
                break;
        }
