...
@WebServlet(name = "TriangleController", urlPatterns = {"/draw/triangle"})
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (acceptHeader) {
            case "image/webp,image/apng,image/*,*/*;q=0.8":
                ImageHelper h = null;
                try {
                    ShapeParams shapeParams = ShapeServletBase.validateShape(request, response, Triangle.class);
                    if (shapeParams.isValid()) {
                        h = new TriangleHelper();
                        ((TriangleHelper) h).Draw((Triangle) shapeParams.getShape());
                ...
                break;
            default:
                ShapeServletBase.processHTMLRequest(request, response, new ShapeParams().setNewShape(new Triangle()), false, loginHelper.ValidateRequest(request), loginHelper.ValidateTeacher(request));
                break;
        }
